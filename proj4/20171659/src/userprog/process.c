...
handle_mm_fault(struct vm_entry *vme)
{
  ...
  if(new_page == NULL)
    ...
    /* if vme is already loaded, return false */
...
process_execute (const char *file_name) 
{
  ...
  /*modified: parse the file_name*/
  char *next_ptr;
  char copied[1024];
  strlcpy(copied, file_name, strlen(file_name) + 1);
  char *argv = strtok_r(copied, " ", &next_ptr);
  ...
  for(temp = list_begin(&(thread_current()->list_child)); 
      temp != list_end(&(thread_current()->list_child));
      temp = list_next(temp)){
        cur = list_entry(temp, struct thread, elem_child);
...
process_exit (void)
{
  ...
  if (pd != NULL) 
    ...
    /*modified: add semaphore function*/
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /*modified: construct stack*/
  int total_len = 0, len;
  for(i = argc - 1; i >= 0; i--){
    len = strlen(argv[i]) + 1;
    *esp -= len; total_len += len;
