...
thread_wake_up(void)
{
  ...
  for(cur_elem = list_begin(&alarm_list); cur_elem != list_end(&alarm_list);){
    cur_thread = list_entry(cur_elem, struct thread, elem);
    if(cur_thread->wake <= timer_ticks()){
...
update_load_avg_and_recent_cpu(void) 
{
  ...
  for (cur_elem = list_begin(&all_list); cur_elem != list_end(&all_list); 
      cur_elem = list_next(cur_elem)) {
    cur_thread = list_entry(cur_elem, struct thread, allelem);
      if (cur_thread != idle_thread)
        cur_thread->recent_cpu = ((int64_t)((int64_t)2 * load_avg) * FRACTION 
        / (2*load_avg + (1 * FRACTION))) * cur_thread->recent_cpu / FRACTION + (cur_thread->nice) * FRACTION;
...
update_priority(void) 
{
  ...
  for (cur_elem = list_begin(&all_list); cur_elem != list_end(&all_list); cur_elem = list_next(cur_elem)) {
    cur_thread = list_entry(cur_elem, struct thread, allelem);
    ...
    cur_thread->priority = ((PRI_MAX * FRACTION) - ((cur_thread->recent_cpu) / 4) - ((cur_thread->nice * 2) * FRACTION))>>14;
    if (cur_thread->priority > PRI_MAX) cur_thread->priority = PRI_MAX;
    if (cur_thread->priority < PRI_MIN) cur_thread->priority = PRI_MIN;
  }

  if (thread_current()->priority < find_max_priority())
...
thread_set_nice (int new_nice) 
{
  ...
  //cur_thread->priority = float_sub_float(float_sub_float(float_add_int(0, PRI_MAX), float_div_int(t->recent_cpu, 4)), int_mul_float(2, float_add_int(0, t->nice))) / FRACTION;
  ...
  if (cur_thread->priority < find_max_priority()) 
    thread_yield();
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  intr_set_level (old_level);
  /*modified3 : priority aging*/
  t->recent_cpu = running_thread()->recent_cpu;
  t->nice = running_thread()->nice; 
  t->wake = 0;
  //thread_set_nice(running_thread()->nice); 

/*modified: to make wait()*/
#ifdef USERPROG
  ...
  /*modified2: initialize the file descriptor*/
  t->fd_num = 3;
  for(int i = 0; i < 128; i++){
    t->fd[i] = NULL;
  }
  /*modified2: sync the file system*/
